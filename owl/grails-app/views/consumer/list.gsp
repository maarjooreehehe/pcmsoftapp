			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="username" title="${message(code: 'consumer.username.label', default: 'Username')}" />
				...
				<g:each in="${consumerInstanceList}" status="i" var="consumerInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${consumerInstance.id}">${fieldValue(bean: consumerInstance, field: "username")}</g:link></td>
					
						<td>${fieldValue(bean: consumerInstance, field: "password")}</td>
